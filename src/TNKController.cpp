...
bool TNKController::initialize(SimpleControllerIO* io)
{
    ...
    joint_names_.push_back("PUSH ROD");
...
void TNKController::stop()
{
    ROS_WARN("TNKController : Stop");
}

void TNKController::velocityCallback(const geometry_msgs::Twist& msg)
{
    twist_mtx_.lock();
    cmd_twist_ = msg;
    twist_mtx_.unlock();
}
void TNKController::modeCallback(const std_msgs::String &msg)
{
    mode_mtx_.lock();
    cmd_mode_ = msg;
    mode_mtx_.unlock();
}
void TNKController::joint_stateCallback(const sensor_msgs::JointState &msg)
{
    joint_state_mtx_.lock();
    cmd_js_ = msg;
    joint_state_mtx_.unlock();
}
void TNKController::joint_trajectoryCallback(const trajectory_msgs::JointTrajectory &msg)
{
...
vector<double> TNKController::calcCMDVel(const double linear, const double angular)
{
...
bool TNKController::control()
{
    twist_mtx_.lock();
    vector<double> cmd_vel = calcCMDVel(cmd_twist_.linear.x, cmd_twist_.angular.z);
    twist_mtx_.unlock();
